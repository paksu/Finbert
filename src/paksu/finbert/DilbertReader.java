...
public class DilbertReader {
	...
	public void nextDay() {
		...
		if(dt.getDayOfWeek() > 5) {
			while ( dt.getDayOfWeek() != 1 ) dt = dt.plusDays(1);
		}
	}
	...
	private String peekNextDate() {
		...
		if(nextDay.getDayOfWeek() > 5) {
			while ( nextDay.getDayOfWeek() != 1 ) nextDay = nextDay.plusDays(1);
		}
		return Integer.toString(nextDay.getYear()) + "-" + Integer.toString(nextDay.getMonthOfYear()) + "-" + Integer.toString(nextDay.getDayOfMonth());
	}
	
	private String peekPreviousDate() {
		DateTime previousDay = dt;
		previousDay = previousDay.minusDays(1);
		while (previousDay.getDayOfWeek() > 5 ) previousDay = previousDay.minusDays(1);
		return Integer.toString(previousDay.getYear()) + "-" + Integer.toString(previousDay.getMonthOfYear()) + "-" + Integer.toString(previousDay.getDayOfMonth());
	}
	...
	private boolean isDilbertAvailable(String dateString) {
		...
		if(availabilityCache.containsKey(dateString)) {
			...
			Log.d("finbert","isDilbertAvailable found cached value for date:" + dateString);
			return cached;
		} else {
